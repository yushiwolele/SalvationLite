...
class ScriptService():
    ...
    def get_all_script_component_path_data(self,tags):
        '''
        获取脚本用例套件/基线包的信息
        :return:
        '''
        sql = "select `id`,`name`,`parent_id`,`level`,`path`,`type` from testcase_script_config where tags={0}".format(tags)
    ...
    def get_script_content_by_path(self,script_path,script_tags):
        ...
        if script_tags == 0: #脚本套件
            ...
            script_path_absolute = os.path.join(self.script_baseline_component_root_path, script_path)

        if script_path_absolute and os.path.isfile(script_path_absolute): #地址存在，可以为空，但不能为none，否则报错
            #读取文件内容
            try:
                if script_path_absolute.endswith(image_suffix): #图片
    ...
    def upload_script_baseline_folder(self,script_baseline_files):
        ...
        root_dir = os.path.join(self.script_baseline_component_root_path, os.path.splitext(script_baseline_files.name)[0])# 解压后的根目录 F:\Salvation学习\脚本功能用到\配置_基线包\IUMP-BSQ-AutoTest-F
        ...
        try:
            with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
                for file_info in zip_ref.infolist():
                    ...
                    zip_ref.extract(file_info, self.script_baseline_component_root_path)
        except:
            try:
                with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
                    for file_info in zip_ref.infolist():
