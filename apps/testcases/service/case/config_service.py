...
class ConfigService:
    ...
    def get_config_file(self,tags):
        sql = "select id,config_file_name,config_file_type,batch_time from testcase_configfile where tags='{0}'".format(tags)
        ...
        for item in result:
            ...
            if item[3]:
                ...
                batch_time = None
            config_list.append({'config_id':config_id,'config_name': config_name, 'config_type': config_type, 'batch_time': batch_time})
        return config_list
    ...
    def download_config_file(self,config_flag,tags):
        ...
        if tags == '0':
            ...
            file_model = TestCaseConfigModel.objects.filter(config_file_name=config_flag, tags=tags)
            file_path_root = self.cfg_read.read_cfg_get_value_by_key('config_file', 'file_template_path')
